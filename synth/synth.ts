    ...
    private static chipSynth(synth: Synth, bufferIndex: number, roundedSamplesPerTick: number, tone: Tone, instrumentState: InstrumentState): void {
        ...
        if (filterCount === 0) {
            for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
                ...
                if (aliases) {
                    inputSample = wave[(0 | phaseA) % waveLength] + wave[(0 | phaseB) % waveLength] * unisonSign;
                } else {
                    const phaseAInt: number = phaseA | 0;
                    const phaseBInt: number = phaseB | 0;
                    const indexA: number = phaseAInt % waveLength;
                    ...
                    inputSample = (nextWaveIntegralA - prevWaveIntegralA) * invPhaseDeltaA + (nextWaveIntegralB - prevWaveIntegralB) * invPhaseDeltaB * unisonSign;
                    prevWaveIntegralA = nextWaveIntegralA;
                    prevWaveIntegralB = nextWaveIntegralB;
                }
                ...
                data[sampleIndex] += inputSample * volumeScale * expression;
                expression += expressionDelta;
            }
        } else {
            for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
                ...
                const inputSampleWithVolume: number = inputSample * volumeScale;
                const sample: number = applyFilters(inputSampleWithVolume, initialFilterInput1, initialFilterInput2, filterCount, filters);
                initialFilterInput2 = initialFilterInput1;
                initialFilterInput1 = inputSampleWithVolume;
    ...
    private static harmonicsSynth(synth: Synth, bufferIndex: number, roundedSamplesPerTick: number, tone: Tone, instrumentState: InstrumentState): void {
        ...
        if (filterCount === 0) {
            for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
                ...
                nextWaveIntegralA += (wave[indexA + 1] - nextWaveIntegralA) * phaseRatioA;
                nextWaveIntegralB += (wave[indexB + 1] - nextWaveIntegralB) * phaseRatioB;
                const inputSample: number = (nextWaveIntegralA - prevWaveIntegralA) * invPhaseDeltaA + (nextWaveIntegralB - prevWaveIntegralB) * invPhaseDeltaB * unisonSign;
                prevWaveIntegralA = nextWaveIntegralA;
                prevWaveIntegralB = nextWaveIntegralB;
